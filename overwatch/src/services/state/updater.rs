...
///
/// Update the current state and notifies the [`StateHandle`](crate::services::state::StateHandle).
...
impl<State> StateUpdater<State> {
    ...
    /// Send a new state and notify the [`StateWatcher`](crate::services::state::StateWatcher).
    ///
    /// `None` values won't be forwarded to the [`StateOperator`](crate::services::state::StateOperator).
